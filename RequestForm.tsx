...
function beneficiaryFromPayee(payee?: Payee): Beneficiary {
  return {
    ...
    bank_country: payee?.bank_country ?? "",
    bank_city: payee?.bank_city ?? "",
    bank_street: payee?.bank_street ?? "",
    bank_postal: payee?.bank_postal ?? "",
    origin: payee?.origin ?? "",
    shipping_country: payee?.shipping_country ?? "",
    shipping_city: payee?.shipping_city ?? "",
    charge_bearer: payee?.charge_bearer ?? ""
  };
}
  ...
  return (
    <form action={createRemittanceRequest} className="panel request-document" onSubmit={validateSubmit}>
      ...
      <section className="document-section">
        <h3>4. 受取銀行・船積情報</h3>
        <div className="form-grid compact">
          <label>受取銀行 国<input name="bank_country" onChange={(event) => updateBeneficiary("bank_country", event.target.value)} value={beneficiary.bank_country} /></label>
          <label>受取銀行 都市<input name="bank_city" onChange={(event) => updateBeneficiary("bank_city", event.target.value)} value={beneficiary.bank_city} /></label>
          <label>受取銀行 通り・番地<input name="bank_street" onChange={(event) => updateBeneficiary("bank_street", event.target.value)} value={beneficiary.bank_street} /></label>
          <label>受取銀行 郵便番号<input name="bank_postal" onChange={(event) => updateBeneficiary("bank_postal", event.target.value)} value={beneficiary.bank_postal} /></label>
          <label>原産地<input name="origin" onChange={(event) => updateBeneficiary("origin", event.target.value)} value={beneficiary.origin} /></label>
          <label>
            手数料負担区分
            <select name="charge_bearer" onChange={(event) => updateBeneficiary("charge_bearer", event.target.value)} value={beneficiary.charge_bearer}>
              <option value="">選択してください</option>
              <option>送金人負担</option>
              <option>受取人負担</option>
              <option>折半</option>
          ...
          <label>船積地 国<input name="shipping_country" onChange={(event) => updateBeneficiary("shipping_country", event.target.value)} value={beneficiary.shipping_country} /></label>
          <label>船積地 都市<input name="shipping_city" onChange={(event) => updateBeneficiary("shipping_city", event.target.value)} value={beneficiary.shipping_city} /></label>
        </div>
      </section>
